...
(function () {
   ...
   * ================================================================== */
  var listState = {
    q: '', category: '', severity: '', emirate: '', provider: '', solution: '',
  ...
  function viewDashboard() {
    ...
    var kpis = [
      { label: 'عدد الفعاليات', value: fmt(m.count), unit: '',
        sub: m.approved + ' معتمد • ' + m.generated + ' مولَّد • ' + m.unsolved + ' بلا حل', cls: '' },
      { label: 'إجمالي المورد', value: fmt(m.delivered), unit: 'كغ',
        sub: 'المستهلك ' + fmt(m.consumed) + ' كغ', cls: '' },
      { label: 'إجمالي المتلف', value: fmt(m.wasted), unit: 'كغ',
        sub: 'الفارق عن الهدف ' + fmt(Math.max(0, m.wasted - Math.round(m.delivered * L.WASTE_TARGET_PCT / 100))) + ' كغ',
        cls: 'is-alert' },
      { label: 'نسبة الهدر المرجحة', value: fmt1(m.wastePct), unit: '%',
        sub: 'الهدف < ' + L.WASTE_TARGET_PCT + '% — الفجوة ' + fmt1(gap) + ' نقطة',
        cls: m.wastePct > L.WASTE_TARGET_PCT ? 'is-alert' : 'is-good' },
      { label: 'كلفة الهدر', value: fmt(m.cost), unit: 'د.إ',
        sub: 'متوسط ' + fmt(Math.round(m.cost / (m.count || 1))) + ' د.إ للفعالية', cls: 'is-gold' },
      { label: 'حوادث عالية/حرجة', value: fmt(m.highOrCritical), unit: 'حادثة',
    ...
    var monthPoints = months.map(function (k) {
      ...
      lastYear = year;
      return { label: monthLabel(k), short: short, value: m.byMonth[k].wasted, color: '#1e3a5f' };
    });

    /* الخطورة */
    var sevColors = { 'حرجة': '#b91c1c', 'عالية': '#ea580c', 'متوسطة': '#ca8a04', 'منخفضة': '#15803d' };
    var sevSlices = ['حرجة', 'عالية', 'متوسطة', 'منخفضة'].map(function (s) {
      return { label: s, value: m.severity[s], color: sevColors[s] };
    });

    /* الإمارات والمزودون */
    function bucketRows(bucket, palette) {
      return Object.keys(bucket).map(function (k, i) {
        return { label: k, value: bucket[k].wasted, color: palette[i % palette.length],
                 sub: bucket[k].count + ' حادثة' };
      }).sort(function (a, b) { return b.value - a.value; });
    }
    var pal = ['#1e3a5f', '#2b4f7e', '#0891b2', '#7c3aed', '#a8791b', '#15803d'];
    ...
    return '' +
      ...
      '<div class="grid grid-2">' +
        '<div class="card">' +
          '<div class="card-head"><h3>الهدر حسب الفئات الست</h3><span class="hint">كغ متلفة</span></div>' +
          '<div class="card-body">' + C.barsH(catRows, { unit: 'كغ', title: 'الهدر حسب الفئة' }) + '</div>' +
        '</div>' +
        '<div class="card">' +
          '<div class="card-head"><h3>كلفة الهدر حسب الفئة</h3><span class="hint">درهم إماراتي</span></div>' +
      ...
      '<div class="grid grid-2 mt">' +
        '<div class="card">' +
          '<div class="card-head"><h3>الهدر عبر الزمن</h3><span class="hint">كغ متلفة شهرياً • الخط الذهبي اتجاه</span></div>' +
          '<div class="card-body">' +
            (monthPoints.length
        ...
        '</div>' +
        '<div class="card">' +
          '<div class="card-head"><h3>توزيع الخطورة</h3><span class="hint">حسب مصفوفة المحاور الثلاثة</span></div>' +
          '<div class="card-body">' +
      ...
      '<div class="grid grid-2 mt">' +
        '<div class="card">' +
          '<div class="card-head"><h3>الهدر حسب الإمارة</h3></div>' +
          '<div class="card-body">' + C.barsH(bucketRows(m.byEmirate, pal), { unit: 'كغ' }) + '</div>' +
        '</div>' +
        '<div class="card">' +
          '<div class="card-head"><h3>الهدر حسب مزود التموين</h3></div>' +
          '<div class="card-body">' + C.barsH(bucketRows(m.byProvider, pal), { unit: 'كغ' }) + '</div>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="card"><div class="card-body stack">' +
        ...
        '<div class="kpi-row"><span>الفعاليات التي تحمل حلاً</span><b>' + m.solved + ' من ' + m.count +
          ' (' + pct(m.solved, m.count) + '%)</b></div>' +
        '<div class="kpi-row"><span>الحلول المولَّدة التي اعتُمدت</span><b>' + m.approved + ' معتمد • ' +
      ...
      '<div class="card mb"><div class="card-body">' +
        '<div class="filters">' +
          ...
          '<div class="field" style="min-width:auto"><label>&nbsp;</label>' +
            '<button class="btn" id="f-clear">مسح المرشحات</button></div>' +
        '</div>' +
      '</div></div>' +
  ...
  function viewNewEvent() {
    ...
    return '' +
      '<div class="split mb">' +
        ...
        '<span class="chip">تُحفظ في التخزين المحلي بنفس مخطط JSON الأصلي</span>' +
      '</div>' +
  ...
  function bindGlobalEvents() {
    document.addEventListener('click', function (e) {
      ...
      if (t.closest('#f-clear')) {
        listState.q = ''; listState.category = ''; listState.severity = '';
        listState.emirate = ''; listState.provider = ''; listState.solution = '';
        render();
        return;
      }
    ...
    document.addEventListener('change', function (e) {
      var map = { 'f-cat': 'category', 'f-sev': 'severity', 'f-emi': 'emirate',
                  'f-prov': 'provider', 'f-sol': 'solution' };
