...
export function MiniPlot({
  ...
  for (let gx = Math.ceil(xMin); gx <= xMax; gx += Math.max(1, Math.round((xMax - xMin) / 10))) {
    gridLines.push(
      <line key={`vx${gx}`} x1={sx(gx)} y1={0} x2={sx(gx)} y2={H} stroke="rgba(244,239,229,0.07)" strokeWidth="1" />
    );
  }
  for (let gy = Math.ceil(yMin); gy <= yMax; gy += Math.max(1, Math.round((yMax - yMin) / 10))) {
    gridLines.push(
      <line key={`hy${gy}`} x1={0} y1={sy(gy)} x2={W} y2={sy(gy)} stroke="rgba(244,239,229,0.07)" strokeWidth="1" />
    );
  }
  ...
  return (
    <div dir="ltr">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full border border-hairline bg-ink" role="img" aria-label={label ?? "graph"}>
        {gridLines}
        {/* axes */}
        {yMin <= 0 && yMax >= 0 && <line x1={0} y1={sy(0)} x2={W} y2={sy(0)} stroke="rgba(244,239,229,0.35)" strokeWidth="1" />}
        {xMin <= 0 && xMax >= 0 && <line x1={sx(0)} y1={0} x2={sx(0)} y2={H} stroke="rgba(244,239,229,0.35)" strokeWidth="1" />}
        <polyline points={pts.join(" ")} fill="none" stroke="#FFCB58" strokeWidth="2.5" />
      </svg>
