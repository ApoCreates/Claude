...
function keyMagenta(src, w, h) {
  ...
  } catch (e) { return src; }   // CORS-tainted: use the raw image rather than crash
}
function loadAsset(n) {
  ...
  local.onerror = () => {
    ...
    im.onload = () => { IMG[n] = keyMagenta(im, im.naturalWidth, im.naturalHeight); };
    im.src = url;
  };
...
function drawSprite(name, x, y, size, angle) {
  ...
  else { ctx.fillStyle = "#c98"; ctx.beginPath(); ctx.arc(0, 0, size / 2, 0, 7); ctx.fill(); }
  ctx.restore();
}
